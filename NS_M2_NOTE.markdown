...
NS-M2 is a conservative extension of the J-ECOT-M2 path for noisy scalogram
episodes. It keeps the M2 backbone, fixed rho budget, threshold mass-cost
...
DeepEMD is strong under clutter because its local matching weights can reduce
background influence. NS-M2 targets the same failure mode with a different,
lower-variance mechanism: a dustbin/noise sink in the ECOT solve.
