...
class FGWUOTFewShot(BaseConv64FewShotModel):
    """Few-shot classifier using FGW-UOT with optional J-style shot pooling."""
    ...
    def _solve_pair_batch(
        ...
        D_q = normalize_intra_dist(pairwise_sq_l2(query_tokens, query_tokens), self.eps)
        D_s = normalize_intra_dist(pairwise_sq_l2(support_tokens, support_tokens), self.eps)
        ...
        transport_distance = transport_cost / rho.clamp_min(self.eps)
        return {
            "C_feat": C_feat,
            "D_q": D_q,
            "D_s": D_s,
    ...
    def _pool_shot_scores(
        ...
        shot_transported_mass: torch.Tensor,
        shot_rho: torch.Tensor,
        shot_distance: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        if shot_logits.shape[-1] == 1:
            weights = torch.ones_like(shot_logits)
            return (
                shot_logits.squeeze(-1),
                shot_transport_cost.squeeze(-1),
                shot_transported_mass.squeeze(-1),
                shot_rho.squeeze(-1),
                shot_distance.squeeze(-1),
                weights,
            )
        ...
        transported_mass = (weights * shot_transported_mass).sum(dim=-1)
        rho = (weights * shot_rho).sum(dim=-1)
        class_distance = (weights * shot_distance).sum(dim=-1)
        return logits, transport_cost, transported_mass, rho, class_distance, weights
    ...
    def _forward_episode(
        ...
        support_tokens = self._encode_tokens(support.reshape(way_num * shot_num, *support.shape[2:]))
        ...
        pair_out = self._solve_pair_batch(query_batch, support_batch)
        ...
        shot_distance = pair_out["transport_distance"].reshape(pair_shape)
        shot_logits = self.score_scale * (-shot_distance)
        ...
        result: Dict[str, Any] = {
            ...
            "transported_mass": transported_mass.detach().reshape(num_query, way_num),
